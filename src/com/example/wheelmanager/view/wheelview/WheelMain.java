...
public class WheelMain {
	...
	public void initDateTimePicker(Context context, int year, int month, int day) {
		this.initDateTimePicker(context, year, month, day, 0, 0);
	}
	
	/**
	 * 弹出日期时间选择器
	 */
	public void initDateTimePicker(Context context, int year, int month, int day, int h, int m) {
		...
		if (hasSelectTime)
			...
			textSize = (screenheight / 100) * 3;
		wv_day.TEXT_SIZE = textSize;	//CommonUtil.px2sp(mContext, 30);
	...
	public String getTime() {
		...
		if (!hasSelectTime)
			...
			int intHour = wv_hours.getCurrentItem();
			String hour = intHour<10 ? "0"+intHour : String.valueOf(intHour);
			int intMin = wv_mins.getCurrentItem();
